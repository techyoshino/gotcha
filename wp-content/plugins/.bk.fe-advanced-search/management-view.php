...
<div class="wrap">
	<div id="feas-admin">
		...
		<form action="<?php menu_page_url( 'feas_management' ); ?>&noheader=true" method="post" name="fm" id="fm" onSubmit="return order_check();"> 
			...
			<input type="hidden" name="<?php echo $feadvns_search_current_tab; ?>" value="0" />
					...
					<div class="param-table clearfix">
						<table class="widefat">
							<tr>
								<th>名称</th>
								<td>
									<input type="text" id="<?php echo esc_attr( $feadvns_search_form_name . $manag_no ); ?>" name="<?php echo esc_attr( $feadvns_search_form_name . $manag_no ); ?>" value="<?php echo esc_attr( get_option( $feadvns_search_form_name . $manag_no ) ); ?>" style="width:80%;" />
								</td>
								<th>フォームID</th>
							...
							<tr>
								<th>検索結果から除外</th>
								<td>
									<label for="<?php echo esc_attr( $feadvns_exclude_id . $manag_no ); ?>">記事（ID）</label><br>
									<input id="<?php echo esc_attr( $feadvns_exclude_id . $manag_no ); ?>" type="text" name="<?php echo esc_attr( $feadvns_exclude_id . $manag_no ); ?>" value="<?php echo esc_attr( $exclude_id ); ?>" /><br>
									<label for="<?php echo esc_attr( $feadvns_exclude_term_id . $manag_no ); ?>">ターム（term_id）</label> <br>
									<input id="<?php echo esc_attr( $feadvns_exclude_term_id . $manag_no ); ?>" type="text" name="<?php echo esc_attr( $feadvns_exclude_term_id . $manag_no ); ?>" value="<?php echo esc_attr( get_option( $feadvns_exclude_term_id . $manag_no ) ); ?>" /><br>
		<!--
										...
										************/
										...
										<select id="feadvns_sort_target_cfkey" name="<?php echo esc_attr( $feadvns_sort_target_cfkey . $manag_no ); ?>">
											...
											for ( $i_meta = 0, $cnt_meta = count( $get_metas ); $i_meta < $cnt_meta; $i_meta++ ) {
												$selected = '';
												$cTargetKey = get_option( $feadvns_sort_target_cfkey . $manag_no );
									...
									<div class="feas-members">
										...
										************/
										...
										<select id="feadvns_sort_target_cfkey_2nd" name="<?php echo esc_attr( $feadvns_sort_target_cfkey_2nd . $manag_no ); ?>">
											...
											for ( $i_meta = 0, $cnt_meta = count( $get_metas ); $i_meta < $cnt_meta; $i_meta++ ) {
												$selected = '';
												$cTargetKey = get_option( $feadvns_sort_target_cfkey_2nd . $manag_no );
												if ( $cTargetKey == $get_metas[$i_meta]->meta_key ) {
													$selected = ' selected="selected"';
												}
												?>
							...
							<tr class="widefat alternate <?php echo esc_attr( $addclass_gray ); ?>">
								...
								<td>
									...
									/*============================
									 ...
									 ============================*/ ?>
									<div>
										<label for="<?php echo esc_attr( $cols[3] . $manag_no . "_" . $i ) ?>"></label>
										<input type="text" name="<?php echo esc_attr( $cols[3] . $manag_no . "_" . $i ) ?>" id="<?php echo esc_attr( $cols[3] . $manag_no . "_" . $i ) ?>" style="width:100%;" value="<?php echo esc_attr( data_to_post( $cols[3] . $manag_no . "_" . $i ) ); ?>">
									</div>
								</td>				
								<td>
									<?php
									/*============================
										条件
									 ============================*/	?>
									<select data-visible-ctl-current="" class="trigger sTerm n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[2] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[2] . $manag_no . "_" . $i ); ?>" style="float:left;">
										
										<?php
										$selected = '';
										if ( "archive" == data_to_post( $cols[2] . $manag_no . "_" . $i ) ) {
											$selected = ' selected="selected"';
										}
										?>
										...
										<?php
										$selected = '';
										if ( 'sel_tag' == data_to_post( $cols[2] . $manag_no . "_" . $i ) ) {
											$selected = ' selected="selected"';
										}
										?>
					                    <optgroup label="タグ">
										...
										<optgroup label="カスタムフィールド">
											...
											for ( $i_meta = 0, $cnt_meta = count( $get_metas ); $i_meta < $cnt_meta; $i_meta++ ) {
												...
												$selected = '';
												if ( 'meta_' . $get_metas[$i_meta]->meta_key == data_to_post( $cols[2] . $manag_no . "_" . $i ) ) {
													$selected = ' selected="selected"';
													$cnt_sel++;
												}
												?>
												<option value="meta_<?php echo esc_attr( $get_metas[$i_meta]->meta_key ); ?>"<?php echo $selected; ?> data-visible-ctl="d"><?php echo esc_attr( $get_metas[$i_meta]->meta_key ); ?></option>
												<?php 
											} 
											// 設定中のカスタムフィールドの値が0件になった場合、カッコ書きでカスタムフィールドのキーを表示
											if ( 'meta_' == substr( $_POST[$cols[2].$manag_no."_".$i], 0, 5 ) && 0 == $cnt_sel ) : 
												?>
												<option value="<?php echo esc_attr( $_POST[$cols[2].$manag_no."_".$i] ); ?>" selected="selected" data-visible-ctl="d">(<?php echo esc_attr( substr( $_POST[$cols[2].$manag_no."_".$i], 5 ) ); ?>)</option>
												<?php 
											endif; 
											?>
										</optgroup>
								...
								<td>
									...
									/*============================
										形式
									 ============================*/	?>
									<select data-visible-ctl-current="" class="trigger sForm n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[4] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[4] . $manag_no . "_" . $i ); ?>">
										<?php
										...
										foreach ( $op_keys as $key => $value ) {
											...
											?>
											<option value="<?php echo esc_attr( $key ); ?>"<?php echo $selected; ?> data-visible-ctl="<?php echo $key; ?>"><?php echo esc_attr( $value ); ?></option>
											<?php
										}
										?>
									</select>
								...
								<td>
									...
									/*============================
										項目内の並び順
									 ============================*/	?>	
									<select data-visible-ctl-current="" data-term-ctl="a" data-form-ctl="abcd" class="trigger sOrder arc-order n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[5] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[5] . $manag_no . "_" . $i ); ?>_arc"  disabled="disabled">
										<?php
										...
										foreach ( $op_keys as $key => $value ) {
											...
											?>
											<option value="<?php echo esc_attr( $key ); ?>"<?php echo $selected; ?> data-visible-ctl="<?php echo $key; ?>"><?php echo esc_html( $value ); ?></option>
											<?php
										}
										?>
									</select>
									
									<select data-visible-ctl-current="" data-term-ctl="bc" data-form-ctl="abcd" class="trigger sOrder term-order n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[5] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[5] . $manag_no . "_" . $i ); ?>_cat" disabled="disabled">
										<?php
									...
									<select data-visible-ctl-current="" data-term-ctl="d" data-form-ctl="abcd" class="trigger sOrder meta-order n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[5] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[5] . $manag_no . "_" . $i ); ?>_meta" disabled="disabled">
										<?php
									...
									?>
									<select id="<?php echo esc_attr( $cols[34] . $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[34] . $manag_no . "_" . $i ); ?>" class="ctl" data-term-ctl="d" data-form-ctl="abcd" data-order-ctl="hi">
										<option value="int"<?php echo $selected_1; ?>>数値</option>
										<option value="str"<?php echo $selected_2; ?>>文字列</option>
									</select>
									...
									?>
									<select id="<?php echo esc_attr( $cols[35] . $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[35] . $manag_no . "_" . $i ); ?>" class="ctl" data-term-ctl="abcd" data-form-ctl="abcd" data-order-ctl="acdehi">
										<option value="asc"<?php echo $selected_1; ?>>昇順</option>
										<option value="desc"<?php echo $selected_2; ?>>降順</option>
									</select>
								</td>				
								<td>
									...
									/*============================
										複数選択時の検索方法
									 ============================*/	?>
									<select id="<?php echo esc_attr( $cols[6] . $manag_no . "_" ); ?><?php echo esc_attr( $i ); ?>" name="<?php echo esc_attr( $cols[6] . $manag_no . "_" . $i ); ?>" class="ctl sAndor n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="abcd" data-form-ctl="bc" data-order-ctl="abcdefghij">
										<?php
									...
									<div class="ele-wrap">
										<div class="wrap-left">
											...
											/*============================
											 ...
											 ============================*/	
											?>									
											<div id="cat_more_setting_<?php echo esc_attr( $i ); ?>" class="ctl term-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="bc" data-form-ctl="abcd" data-order-ctl="acdefghij">
												<div class="ele-title">
													<h3>ターム詳細項目</h3>
												</div>
												<div class="ele-content">
													...
													?>
													<div id="term-ex_<?php echo esc_attr( $i ); ?>" class="ctl" data-term-ctl="bc" data-form-ctl="abcd" data-order-ctl="acdefghij">
														<h4>除外ID</h4> 
														<input type="text" name="<?php echo esc_attr( $cols[11] . $manag_no . "_" . $i ); ?>" value="<?php echo esc_attr( $_POST[$cols[11] . $manag_no . "_" . $i] ); ?>" />
													</div>
													<div id="term-depth_<?php echo esc_attr( $i ); ?>" class="ctl" data-term-ctl="bc" data-form-ctl="abcd" data-order-ctl="acdefghij">
														<h4>階層</h4>  
														<input type="text" name="<?php echo esc_attr( $cols[10] . $manag_no . "_" . $i ); ?>" value="<?php echo esc_attr( data_to_post( $cols[10] . $manag_no . "_" . $i ) ); ?>" />
													</div>
												</div>
											</div>
											<div id="cat_more_setting_<?php echo esc_attr( $i ); ?>_2nd" class="ctl term-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="bc" data-form-ctl="abcd" data-order-ctl="acdefghij">
												<div class="ele-content">
													<?php
													$emptycat_checked = '';
													if ( 'no' == get_option( $cols[14] . $manag_no . "_" . $i ) ) {
														$emptycat_checked = ' checked="checked"';
													}
													?>
													<label id="cat_more_setting_<?php echo esc_attr( $i ); ?>_2nd_1" for="<?php echo esc_attr( $cols[14] . $manag_no . "_" . $i ); ?>" class="ctl empty n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="bc" data-form-ctl="abcd" data-order-ctl="cdefg">
														<input id="<?php echo esc_attr( $cols[14] . $manag_no . "_" . $i ); ?>" type="checkbox" name="<?php echo esc_attr( $cols[14] . $manag_no . "_" . $i ); ?>" value="no"<?php echo $emptycat_checked; ?> class="ctl n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="bc" data-form-ctl="abcd" data-order-ctl="cdefg">&nbsp;登録件数が0件のカテゴリ/タームは表示しない
													</label>
													
													<?php
													$emptycat_checked = '';
													if ( 'no' == get_option( $cols[19] . $manag_no . "_" . $i ) ) {
														$emptycat_checked = ' checked="checked"';
													}
													?>
													<label id="cat_more_setting_<?php echo esc_attr( $i ); ?>_2nd_2" for="<?php echo esc_attr( $cols[19] . $manag_no . "_" . $i ); ?>" class="ctl ajax n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="c" data-form-ctl="a" data-order-ctl="cdefg">
														<input id="<?php echo esc_attr( $cols[19] . $manag_no . "_" . $i ); ?>" type="checkbox" name="<?php echo esc_attr( $cols[19] . $manag_no . "_" . $i ); ?>" value="no"<?php echo $emptycat_checked; ?> class="ctl n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="c" data-form-ctl="a" data-order-ctl="cdefg">&nbsp;Ajaxフィルタリング
													</label>
											...
											/*============================
											 ...
											 ============================*/	
											?>						
											<div id="<?php echo esc_attr( $cols[13] . $manag_no . "_" . $i ); ?>"  class="ctl keyword-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="_abcd" data-form-ctl="e" data-order-ctl="_abcdefghij">
												<div class="ele-title">
													<h3>フリーワード詳細項目</h3>
												</div>
												<div class="ele-content">
													...
													for ( $i_t = 0; $i_t < 6; $i_t++ ) {
														...
														?>
														<label>
															<input type="checkbox" name="<?php echo esc_attr( $cols[13] . $manag_no . "_" . $i ); ?>[]" id="<?php echo esc_attr( $cols[13] . $manag_no . "_" . $i . "_" . $i_t ); ?>" value="<?php echo esc_attr( $k_data[$i_t]['value'] ); ?>"<?php echo $kwds_checked[$i_t]; ?>/>&nbsp;<?php echo esc_attr( $k_data[$i_t]['name'] ); ?>
														</label>
														...
														<?php if ( 3 == $i_t ) {
															...
															<p style="padding-left: 3em;">
																<label>キーを限定（meta_key）&nbsp;
																	<input type="text" name="<?php echo esc_attr( $cols[20] . $manag_no . "_" . $i ); ?>" value="<?php echo esc_attr( $kwds_keys ) ?>" style="width: 60%" />
																</label>
															</p>
															<?php 
														}
												...
												<div class="ele-content">
													<div id="<?php echo esc_attr( $cols[15] . $manag_no . "_" . $i ); ?>">
														...
														<?php
														$kwds_yuragi_checked = '';
														if ( 'no' == get_option( $cols[15] . $manag_no . "_" . $i ) ) {
															$kwds_yuragi_checked = ' checked="checked"';
														}
														?>
														<label>
															<input type="checkbox" name="<?php echo esc_attr( $cols[15] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[15] . $manag_no . "_" . $i ); ?>" value="no"<?php echo $kwds_yuragi_checked; ?> />&nbsp;半角/全角、ひらがな/カタカナを区別しない
														</label>	
													</div>
												</div>
											</div>
															
											<div id="<?php echo esc_attr( $cols[13] . $manag_no . "_" . $i ); ?>_2nd" class="ctl keyword-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="_abcd" data-form-ctl="e" data-order-ctl="_abcdefghij">							
												<div class="ele-content">	
													<h4>プレースホルダーテキスト</h4>
														<label><input type="text" name="<?php echo esc_attr( $cols[30] . $manag_no . "_" . $i ); ?>" value="<?php echo esc_attr( data_to_post( $cols[30] . $manag_no . '_' . $i ) ); ?>" style="width: 80%" /></label>
												</div>
											</div>
											 ...
											 */
											$cf_freeword_checked = '';
											if ( 'yes' == get_option( $cols[22] . $manag_no . "_" . $i ) ) {
												$cf_freeword_checked = ' checked="checked"';
											}
											?>
											
											<div id="cf_range_setting_<?php echo esc_attr( $i ); ?>_0"  class="ctl range-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="ad" data-form-ctl="ad" data-order-ctl="abcdefghij">
												<div class="ele-title">
													<h3>範囲検索</h3>
												</div>
												<div class="ele-content">
													<div>
														<!-- 投稿年月 -->
														<select class="ctl range-arc n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[16] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[16] . $manag_no . "_" . $i ); ?>_0" data-term-ctl="a" data-form-ctl="ad" data-order-ctl="abcdefghij" disabled="disabled">
															<?php echo $output; ?>
														</select>
														<!-- カスタムフィールド -->
														<select class="ctl range-cf n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[16] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[16] . $manag_no . "_" . $i ); ?>_1" data-term-ctl="d" data-form-ctl="ad" data-order-ctl="abcdefghij" disabled="disabled">
															<?php echo $output_2; ?>
														</select>
														<select class="ctl range-as n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[29] . $manag_no . "_" . $i ); ?>" name="<?php echo esc_attr( $cols[29] . $manag_no . "_" . $i ); ?>" data-term-ctl="d" data-form-ctl="ad" data-order-ctl="abcdefghij">
															<option value="int"<?php echo $selected_1; ?>>数値</option>
															<option value="str"<?php echo $selected_2; ?>>文字列</option>
														</select>
													</div>
													<div>
														<label>
															<input id="<?php echo esc_attr( $cols[22] . $manag_no . "_" . $i ); ?>" class="ctl range-by-text n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[29] . $manag_no . "_" . $i ); ?>" type="checkbox" name="<?php echo esc_attr( $cols[22] . $manag_no . "_" . $i ); ?>" value="yes" <?php echo $cf_freeword_checked; ?> data-term-ctl="ad" data-form-ctl="ad" data-order-ctl="abcdefghij">テキスト入力で範囲検索
														</label>
													</div>
												</div>
											</div>
											...
											/*============================
												カスタムフィールド詳細項目
											 ============================*/	?>					
											<div id="cf_more_setting_<?php echo esc_attr( $i ); ?>" class="ctl meta-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="d" data-form-ctl="abcd" data-order-ctl="acdefghij">						
												<div class="ele-title">
													<h3>カスタムフィールド詳細項目</h3>
												</div>
												<div class="ele-content">								
													<div>
														<label>単位を付与する&nbsp;
															<input type="text" name="<?php echo esc_attr( $cols[17] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[17] . $manag_no . "_" . $i ); ?>" style="width: 4em" value="<?php echo esc_attr( data_to_post( $cols[17] . $manag_no . "_" . $i ) ); ?>" >
														</label>
													...
													<?php
													$cf_kugiri_checked = '';
													if ( 'yes' == get_option( $cols[18] . $manag_no . "_" . $i ) ) {
														$cf_kugiri_checked = ' checked="checked"';
													}
													?>
													<div>
														<label>
															<input type="checkbox" name="<?php echo esc_attr( $cols[18] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[18] . $manag_no . "_" . $i ); ?>" value="yes" <?php echo $cf_kugiri_checked; ?> />&nbsp;3桁ごとに半角カンマで区切る
														</label>
													</div>
												</div>
											</div>
																					
											<div id="cf_more_setting_<?php echo esc_attr( $i ); ?>_2nd"	class="ctl meta-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="d" data-form-ctl="abcd" data-order-ctl="acdefghij">
												<div class="ele-content">
													...
													// 真偽値
													$cf_shingi_checked = '';
													if ( '1' == get_option( $cols[33] . $manag_no . "_" . $i ) ) {
														$cf_shingi_checked = ' checked="checked"';
													}
													?>
													<div id="<?php echo esc_attr( $cols[33] . $manag_no . "_" . $i ); ?>_0" class="ctl shingi-more n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="d" data-form-ctl="ad" data-order-ctl="abcdefghij">
														<label>
															<input type="radio" name="<?php echo esc_attr( $cols[33] . $manag_no . "_" . $i ); ?>" value="1" <?php echo $cf_shingi_checked; ?> />&nbsp;真偽値として扱う
														</label>
														<div style="padding-left:3em">
															<label>真の場合の文字列&nbsp;
																<input type="text" name="<?php echo esc_attr( $cols[25] . $manag_no . "_" . $i ); ?>" value="<?php echo esc_attr( data_to_post( $cols[25] . $manag_no . '_' . $i ) ); ?>" style="width: 60%" />
															</label>
														</div>
													</div>
													...
													<?php
													// 関連する投稿
													$cf_related_post_checked = '';
													if ( '2' == get_option( $cols[33] . $manag_no . "_" . $i ) ) {
														$cf_related_post_checked = ' checked="checked"';
													}
													?>
													<div>
														<label>
															<input type="radio" name="<?php echo esc_attr( $cols[33] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[33] . $manag_no . "_" . $i ); ?>_1" value="2" <?php echo $cf_related_post_checked; ?> />&nbsp;関連する投稿
														</label>
													</div>
				
													<?php	
													// 関連するターム
													$cf_related_term_checked = '';
													if ( '3' == get_option( $cols[33] . $manag_no . "_" . $i ) ) {
														$cf_related_term_checked = ' checked="checked"';
													}
													?>
													<div>
														<label>
															<input type="radio" name="<?php echo esc_attr( $cols[33] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[33] . $manag_no . "_" . $i ); ?>_2" value="3" <?php echo $cf_related_term_checked; ?> />&nbsp;関連するターム
														</label>
													</div>
										...
										<div class="wrap-right">
											...
											?>
											<div id="<?php echo esc_attr( $cols[36] . $manag_no . "_" . $i ); ?>" class="ctl disp_op_order_freetext n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="abcd" data-form-ctl="abcd" data-order-ctl="b">
												<div class="ele-title">
													<h3>検索パーツの表示項目（自由記述の内容）</h3>
												</div>
												<div class="ele-content">									
													<h4>要素の選択肢（表記:値）</h4>
													<textarea name="<?php echo esc_attr( $cols[36] . $manag_no . "_" . $i ); ?>" rows="8" cols="30" placeholder="例1）&#13;&#10;1,000円:1000&#13;&#10;2,000円:2000&#13;&#10;5,000円:5000&#13;&#10;&#13;&#10;例2）&#13;&#10;東京都&#13;&#10;--23区&#13;&#10;----千代田区&#13;&#10;----中央区&#13;&#10;----港区"><?php echo esc_attr( $contents ); ?></textarea>
		<!-- 											<button type="button" name="load_current_term" class="button-secondary" value="load">選択中の「条件」から読み込む</button> -->
											...
											<div class="ele-content">
												...
												?>
												<span id="<?php echo esc_attr( $cols[27] . $manag_no . "_" . $i ); ?>" class="ctl noselect-text n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="abcd" data-form-ctl="abcd" data-order-ctl="abcdefghij">
													<h4>未選択時の文字列</h4>
													<input type="text" name="<?php echo esc_attr( $cols[27] . $manag_no . "_" . $i ); ?>" value="<?php echo esc_attr( $text ); ?>" />
												</span>
												...
												/*============================
												 ...
												 ============================*/	
												$noselect_checked = '';
												if ( '1' == get_option( $cols[31] . $manag_no . "_" . $i ) ) {
													$noselect_checked = ' checked="checked"';
												}
												?>							
												<span id="<?php echo esc_attr( $cols[31] . $manag_no . "_" . $i ); ?>" class="ctl noselect-check n<?php echo esc_attr( $manag_no . "_" . $i ); ?>" data-term-ctl="abcd" data-form-ctl="d" data-order-ctl="abcdefghij">
													<label for="<?php echo esc_attr( $cols[31] . $manag_no . "_" . $i ); ?>">表示 
														<input type="checkbox" name="<?php echo esc_attr( $cols[31] . $manag_no ."_" . $i ); ?>" value="1" <?php echo $noselect_checked; ?> />
													</label>
												</span>
											</div>
											...
											<div class="ele-content">
												<div>
													...
													<input type="text" name="<?php echo esc_attr( $cols[7] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[7] . $manag_no . "_" . $i ); ?>" style="width:100%" value="<?php echo esc_attr( data_to_post( $cols[7] . $manag_no . "_" . $i ) ); ?>" >
												</div>
											</div>
											<div class="ele-content">
												<div>
													<h4>後に挿入するHTML/CSS</h4>
													<input type="text" name="<?php echo esc_attr( $cols[8] . $manag_no . "_" . $i ); ?>" id="<?php echo esc_attr( $cols[8] . $manag_no . "_" . $i ); ?>" style="width:100%" value="<?php echo esc_attr( data_to_post( $cols[8] . $manag_no . "_" . $i ) ); ?>" >
												</div>
											</div>
											...
											<div class="ele-content" style="text-align:right">
												<div>
													...
													/*============================
													 ...
													 ============================*/	
													?>							
													<label for="<?php echo esc_attr( $cols[0] . $manag_no . "_" . $i ); ?>">並び順</label>
													...
													?>							
													<label for="<?php echo esc_attr( $cols[1] . $manag_no . "_" . $i ); ?>">表示</label>
													...
													/*============================
													 ...
													 ============================*/	
													?>							
													<label for="<?php echo esc_attr( $cols[9] . $manag_no . "_" . $i ); ?>" class="ele-delete">項目を削除 
														<input type="checkbox" name="<?php echo esc_attr( $cols[9] . $manag_no ."_" . $i ); ?>" id="<?php echo esc_attr( $cols[9] . $manag_no . "_" . $i ); ?>" value="del" />
													</label>
												</div>		
											</div><!-- .wrap-content -->
									...
									<div class="ele-wrap">
										<div class="wrap-right">
											<div class="ele-content">
												<div>
													<h4>前に挿入するHTML/CSS</h4>
													<input type="text" name="<?php echo esc_attr( $feadvns_search_b_label . $manag_no . '_before' ); ?>" style="width: 100%" value="<?php echo esc_attr( data_to_post( $feadvns_search_b_label . $manag_no . '_before' ) ); ?>">
												</div>
											</div>
											<div class="ele-content">
												<div>
													<h4>後に挿入するHTML/CSS</h4>
													<input type="text" name="<?php echo esc_attr( $feadvns_search_b_label . $manag_no . '_after' ); ?>" style="width: 100%" value="<?php echo esc_attr( data_to_post( $feadvns_search_b_label . $manag_no . '_after' ) ); ?>">
												</div>
											</div>
										</div>
									</div>
...
*/
							// 「デザイン」のCSSを読み込むかどうか
							$css_checked = '';
							$load_flag = get_option( $pv_css . $manag_no );
							if ( 'yes' === $load_flag ) {
...
-->
								<input id="<?php echo esc_attr( $pv_css . $manag_no ); ?>" type="checkbox" name="<?php echo esc_attr( $pv_css . $manag_no ); ?>" value="yes" <?php echo $css_checked; ?>>
								<label for="<?php echo esc_attr( $pv_css . $manag_no ); ?>">「デザイン」のCSSを適用する</label>
							</div>
						</div>
						...
						?>
						<div id="feas_pv_lower"><iframe class="" name="feas_pv" src="<?php echo esc_url( $src_url ); ?>?feas_pv=1&amp;feas_mng_no=<?php echo esc_attr( $manag_no ); ?>&amp;feas_pv_type=search" width="100%" marginheight="0" scrolling="auto">プレビュー</iframe></div>
...
<pre>現在の登録件数：&lt;?php feas_search_count(); ?&gt; 件</pre>
						...
						<h4>検索結果のページに検索条件を列記する関数</h4>
						<textarea onfocus="SelectText( this );" style="width: 300px; height: 2em;">feas_search_query()</textarea>
						<h5>記述例</h5>
<pre>「&lt;?php feas_search_query(); ?&gt;」の条件による検索結果 &lt;?php feas_search_count(); ?&gt; 件</pre>
						<h5>ショートコード</h5>
...
<script type="text/javascript">
	...
	jQuery( document ).ready( function( $ ) {
		...
		function block_to_table() {
		    try {
		        $('form#fm tbody tr').css('display', 'table-row');
		    }
		    catch(e) {}
		}
		...
		for ( i = 0; i < max; i++ ) {
			...
			$(".sForm.n"+form_no).data('visible-ctl-current', sFormTemp);
				...
				var _cTarget = $(".sTerm.n"+form_no).closest('tr').find('.sOrder');
				...
				if ( 'e' === sFormTemp ) {
					$(".sTerm.n"+form_no).closest('tr').find('.sTerm').hide();
				} else {
					$(".sTerm.n"+form_no).closest('tr').find('.sTerm').fadeIn();
				}
		...
		$('.trigger').on('change', function(){
			...
			// 現在の各selectのdata-visible-ctl-currentの値を取得
			var sFormTemp = $(this).closest('tr').find('select.sForm option:selected').data('visible-ctl');			
			$(this).closest('tr').find('select.sForm').data('visible-ctl-current', sFormTemp);
			
			// 現在有効化されているselect.sFormのdata-visible-ctl-currentの値を取得
			var sForm  = $(this).closest('tr').find('select.sForm').data('visible-ctl-current');
			...
			if ( ! sForm ) {
				...
				var _cTarget = $(this).closest('tr').find('.sOrder');
				_cTarget.hide();
				...
				// 「形式」が「フリーワード」の場合は、「条件」ドロップダウンを非表示		
				if ( 'e' === sForm ) {
					$(this).closest('tr').find('.sTerm').hide();
				} else {
					$(this).closest('tr').find('.sTerm').fadeIn();
				}	

			// 現在の各selectのdata-visible-ctl-currentの値を取得
			var sOrderTemp = $(this).closest('tr').find('select.sOrder:not(:disabled) option:selected').data('visible-ctl');			
			$(this).closest('tr').find('select.sOrder').data('visible-ctl-current', sOrderTemp);
			
			// 現在有効化されているselect.sOrderのdata-visible-ctl-currentの値を取得
			var sOrder  = $(this).closest('tr').find('select.sOrder:not(:disabled)').data('visible-ctl-current');
