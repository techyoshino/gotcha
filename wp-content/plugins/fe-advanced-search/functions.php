...
function create_child_op( $par_id = null, $term_depth = -1, $class_cnt = 2, $q_term_id = array(), $nocnt = false, $exids = null, $sticky = array(), $showcnt = null, $manage_line = null, $taxonomy = false, $par_no = 0, $number = 0, $sp = array(), $to = " t.term_id ASC ") {
	global $wpdb, $cols, $manag_no, $feadvns_search_target, $feadvns_include_sticky, $feadvns_search_b_label, $feadvns_default_cat;
...
*/
	...
	for ( $i_ele = 0; $i_ele < $cnt_ele; $i_ele++ ) {
		...
		// Sanitaize
		$ret_id    = esc_attr( "feas_{$manag_no}_{$number}_{$par_no}_{$i_ele}" );
		$ret_class = esc_attr( "feas_clevel_{$d_class_cnt}" );
		$ret_val   = esc_attr( $get_cats[$i_ele]->term_id );
		$ret_txt   = esc_html( $get_cats[$i_ele]->name . $cat_cnt );

		$html  = "<option id='{$ret_id}' class='{$ret_class}' value='{$ret_val}' {$attr} {$selected}>";
		$html .= $nbsp . $ret_txt;
		$html .= "</option>\n";

		// 階層の指定がある場合
		if ( $term_depth > 1 ) {
			$html .= create_child_op( $get_cats[$i_ele]->term_id, ( $term_depth - 1 ), ( $class_cnt + 1 ), $q_term_id, $nocnt, $exids, $sticky, $showcnt, $manage_line, $taxonomy, $par_no . '_' . $i_ele, $number, $sp, $to );

		// 階層が未指定(=無制限)の場合
		} else if ( $term_depth === -1 ) {
			$html .= create_child_op( $get_cats[$i_ele]->term_id, $term_depth , ( $class_cnt + 1 ), $q_term_id, $nocnt, $exids, $sticky, $showcnt, $manage_line, $taxonomy, $par_no . '_' . $i_ele, $number, $sp, $to );
		}
...
function create_child_check( $par_id = null, $ele_class = null, $check_cnt = 0, $class_cnt = 2, $nocnt = null, $exids = null, $sticky = array(), $showcnt = null, $taxonomy = false, $par_no = 0, $number = 0, $sp = array(), $to = "t.term_id ASC" ) {
	global $wpdb, $manag_no, $total_cnt, $feadvns_search_target, $feadvns_include_sticky, $feadvns_search_b_label, $feadvns_default_cat;
	...
	for ( $i_ele = 0; $i_ele < $cnt_ele; $i_ele++ ) {
		...
		$html  = "<label for='{$ret_id}' class='{$ret_dclass_cnt}'>";
		...
		// 階層の指定がある場合
		if ( $check_cnt > 1 ) {
			$html .= create_child_check( $get_cats[$i_ele]->term_id, "feas_clevel_", ( $check_cnt - 1 ), ( $class_cnt + 1 ), $nocnt, $exids, $sticky, $showcnt, $taxonomy, $par_no . '_' . $i_ele, $number, $sp, $to );

		// 階層が未指定(=無制限)の場合
		} else if ( $check_cnt == -1 ) {
			$html .= create_child_check( $get_cats[$i_ele]->term_id, "feas_clevel_", $check_cnt, ( $class_cnt + 1 ), $nocnt, $exids, $sticky, $showcnt, $taxonomy, $par_no . '_' . $i_ele, $number, $sp, $to );
		}
...
function create_child_radio( $par_id = null, $ele_class = null, $check_cnt = 0, $class_cnt = 2, $nocnt, $exids, $sticky, $showcnt, $taxonomy = false, $par_no, $number = 0, $sp = array(), $to = "t.term_id ASC" ) {
	global $wpdb, $manag_no, $feadvns_search_target, $feadvns_include_sticky, $feadvns_search_b_label, $feadvns_default_cat;
	...
	for ( $i_ele = 0; $i_ele < $cnt_ele; $i_ele++ ) {
		...
		$html  = "<label for='{$ret_id}' class='{$ret_class}'>";
		...
		// 階層の指定がある場合
		if ( $check_cnt >1 ) {
			$html .= create_child_radio( $get_cats[$i_ele]->term_id, "feas_clevel_", ( $check_cnt - 1 ), ( $class_cnt + 1 ), $nocnt, $exids, $sticky, $showcnt, $taxonomy, $par_no . '_' . $i_ele, $number, $sp, $to );

		// 階層が未指定(=無制限)の場合
		} else if ( $check_cnt == -1 ) {
			$html .= create_child_radio( $get_cats[$i_ele]->term_id, "feas_clevel_", $check_cnt, ( $class_cnt + 1 ), $nocnt ,$exids, $sticky, $showcnt, $taxonomy, $par_no . '_' . $i_ele, $number, $sp, $to );
		}
...
function feas_get_hierarchical_term_list( $args = '' ) {
	...
	if ( false === ( $terms = feas_cache_judgment( $manag_no, 'taxonomy_' . $args['tax_name'], $args['parent'] ) ) ) {
		$term_args = array(
		...
		feas_cache_create( $manag_no, 'taxonomy_' . $args['tax_name'], $args['parent'], $terms );
	}
...
/*============================
	ソート用に連結する
 ============================*/
function feas_join_datas( $get_data = null ) {
...
function feas_print_preview() {
	...
	<html>
		<head>
			...
			<script type="text/javascript" src="<?php echo plugins_url() . '/' . str_replace( basename(__FILE__), "", plugin_basename(__FILE__) ) . 'feas.js'; ?>"></script>
			<style>
...
/*============================
	...
	例）[$i]['meta_key']
 ============================*/
function feas_get_cf_key_list( $formId ) {
	global $wpdb, $feadvns_search_target;
	 ...
	 *	検索対象のpost_typeを取得
	 */
	$savedTarget = get_option( $feadvns_search_target . $formId );
	if ( $savedTarget ) {
		$targetPostType = "'" . implode( "','", (array) $savedTarget ) . "'";
	} else {
		$targetPostType = "'post'";
	}

	if ( false === ( $metaKeys = feas_cache_judgment( $formId, 'meta_keys_of_' . $formId, false ) ) ) {
		...
		$sql .= "WHERE 1=1 ";
		$sql .= "AND p.post_type IN (" . $targetPostType . ") ";
		$sql .= "AND p.post_status IN ('publish','inherit') ";
		$sql .= "AND pm.meta_key NOT LIKE '\_%'";
		$results = $wpdb->get_results( $sql, ARRAY_A );
		if ( $results ) {
			foreach ( $results as $key => $value ) {
				$metaKeys[] = $value['meta_key'];
			}
			$metaKeys = array_values( $metaKeys );
		}

		// ACFのキー一覧
		$sql = "SELECT p.post_excerpt FROM {$wpdb->posts} as p WHERE p.post_type = 'acf-field'";
		$results = $wpdb->get_results( $sql, ARRAY_A );
		if ( $results ) {
			foreach ( $results as $key => $value ) {
				$metaKeys2[] = $value['post_excerpt'];
			}
		}

		$metaKeys = array_merge( $metaKeys, $metaKeys2 );
		$metaKeys = array_unique( $metaKeys );

		feas_cache_create( $formId, 'meta_keys_of_' . $formId, false, $metaKeys );
	}
	return $metaKeys;
}
...
function feas_get_cf_value_list( $meta_key ) {
	...
	if ( false === ( $meta_values = feas_cache_judgment( $manag_no, 'meta_values_of_' . $meta_key, false ) ) ) {
		...
		feas_cache_create( $manag_no, 'meta_values_of_' . $meta_key, false, $meta_values );
	}
	return $meta_values;
}

/**
 *	Judge whether return "selected" or nothing.
 *
 *	@param str $targetStr
 *	@param int $formId
 *	@param int $itemNo
 ...
 *
 */
function feas_selected( $targetStr, $formId, $itemNo, $paramName ) {

	$selected = '';

	/**
	 * 【フォーム全体】の場合は $itemNo にはfalseを渡す
	 * 【検索項目】の場合は行番号を渡し、アンダースコアを付けて$_POST値の形式に整形
	 */
	if ( false !== $itemNo ) {
		$itemNo = "_" . $itemNo;
	}

	if ( $targetStr === data_to_post( $paramName . $formId . $itemNo ) ) {
		$selected = ' selected="selected"';
	}
...
/**
 *	Judge whether return "checked" or nothing.
 *
 *	@param str $targetStr
 *	@param int $formId
 *	@param int $itemNo
 ...
 *
 */
function feas_checked( $targetStr, $formId, $itemNo, $paramName ) {
...
function feas_get_attr_for( $prefix, $formId, $itemNo ){
}
function feas_get_attr_name( $prefix, $formId, $itemNo ){
}
function feas_get_attr_id( $prefix, $formId, $itemNo ){
}
function feas_get_attr_class( $prefix, $formId, $itemNo ){
}
function feas_get_attr_value( $prefix, $formId, $itemNo ){
}
function feas_get_attr_data( $prefix, $formId, $itemNo ){
}

/**
 * Ajaxフィルタリング全体をラップしているdivタグ（過去バージョンとの後方互換のため）。
 *
 * 不要な場合は下記の一行をテーマのfunctions.phpなどに記述すると適用を外すことができる。
 * remove_filter( 'feas_term_ajax_dropdown_group_html', 'feas_term_ajax_wrapper' );
 *
 * @param string $html
 * @param array $cols
 		DB column's prefix
 ...
 *
 */
function feas_term_ajax_wrapper( $html, $args ) {

	global $cols;

	if ( '' === $args['manag_no'] || NULL === $args['manag_no'] ) {
		return $html;
	}

	if ( 1 === $args['ajax'] && -1 === $args['depth'] ) {
		 ...
		 *	未選択時の文字列
		 */
		$noselect_text = esc_attr( get_option( $cols[27] . $manag_no . '_' . $number ) );

		// Sanitize
		$ret_name  = esc_attr( "search_element_{$number}[]" );
		$ret_id    = esc_attr( "ajax_filtering_{$manag_no}_{$number}" );
		$ret_class = esc_attr( "search_element_{$number}" );

		$preHtml  = "<div id='{$ret_id}' class='{$ret_class}' data-feas-label='{$noselect_text}'>\n";
		$postHtml = "</div>\n";
...
add_filter( 'feas_term_ajax_dropdown_group_html', 'feas_term_ajax_wrapper', 10, 2 );
