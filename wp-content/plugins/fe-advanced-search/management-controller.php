...
*/
	 ...
	 *************************************************************************/
 	if ( isset( $_POST['ac'] ) == true && $_POST['ac'] == "update" ) {
		 ...
		 *	検索項目数
		 */
// 		if ( isset( $_POST['line_cnt'] ) ) {
//
// 			$line_cnt = $_POST['line_cnt'];
//
// 			$status = update_option( $feadvns_max_line . $manag_no, $line_cnt );
// 			if ( true == $status ) {
// 				$i++;
// 			}
// 		} else {
//
// 			$status = delete_option( $feadvns_max_line . $manag_no, $line_cnt );
// 			if ( 1 == $status ) {
// 				$i++;
// 			}
// 		}
		 ...
		 *	投稿タイプ毎にソート
		 */
		if ( isset( $_POST[$feadvns_sort_by_posttype . $manag_no] ) ) {

			$sbpt = $_POST[$feadvns_sort_by_posttype . $manag_no];

			$status = update_option( $feadvns_sort_by_posttype . $manag_no , $sbpt );
			if ( true == $status ) {
				$i++;
			}
			...
			$status = delete_option( $feadvns_sort_by_posttype . $manag_no );
			if ( 1 == $status ) {
				$i++;
			}
		}
...
*/
		 ...
		 *	フォームの表示方法
		 */
		if ( isset( $_POST[$feadvns_form_apply_style . $manag_no] ) ) {

			$apply_style = $_POST[$feadvns_form_apply_style . $manag_no];

			$status = update_option( $feadvns_form_apply_style . $manag_no , $apply_style );
			if ( true == $status ) {
				$i++;
			}
			...
			$status = delete_option( $feadvns_form_apply_style . $manag_no );
			if ( 1 == $status ) {
				$i++;
			}
		}
		 ...
		 *	リセットボタン
		 */
		if ( isset( $_POST[$feadvns_reset_btn_sw .$manag_no] ) ) {

			$reset_btn_sw = $_POST[$feadvns_reset_btn_sw . $manag_no];
			$status = update_option( $feadvns_reset_btn_sw . $manag_no, $reset_btn_sw );
			if ( true == $status ) {
				$i++;
			}
			...
			$status = delete_option( $feadvns_reset_btn_sw . $manag_no );
			if ( 1 == $status ) {
				$i++;
			}
		}
		 ...
		 *	リセットボタン | JavaScript
		 */
		if ( isset( $_POST[$feadvns_reset_btn_js .$manag_no] ) ) {

			$reset_btn_js = $_POST[$feadvns_reset_btn_js . $manag_no];
			$status = update_option( $feadvns_reset_btn_js . $manag_no, $reset_btn_js );
			if ( true == $status ) {
				$i++;
			}
			...
			$status = delete_option( $feadvns_reset_btn_js . $manag_no );
			if ( 1 == $status ) {
				$i++;
			}
		}
		 ...
		 *	リセットボタン | 位置
		 */
		if ( isset( $_POST[$feadvns_reset_btn_position . $manag_no] ) ) {

			$reset_btn_position = $_POST[$feadvns_reset_btn_position . $manag_no];
			$status = update_option( $feadvns_reset_btn_position . $manag_no, $reset_btn_position );
			if ( true == $status ) {
				$i++;
			}
			...
			$status = delete_option( $feadvns_reset_btn_position . $manag_no );
			if ( 1 == $status ) {
				$i++;
			}
		}
		 ...
		 *	リセットボタン | 文字列
		 */
		if ( isset( $_POST[$feadvns_reset_btn_text . $manag_no] ) ) {

			$reset_btn_text = $_POST[$feadvns_reset_btn_text . $manag_no];
			$status = update_option( $feadvns_reset_btn_text . $manag_no, $reset_btn_text );
			if ( true == $status ) {
				$i++;
			}
			...
			$status = delete_option( $feadvns_reset_btn_text . $manag_no );
			if ( 1 == $status ) {
				$i++;
			}
		}
...
*/
		 ...
		 *	全体の設定を保存時に他の処理を差し込む
		 */
		do_action( 'feas_admin_management_save_general', $_POST, $manag_no );
...
*/
			...
			if ( isset( $_POST[$cols[27] . $manag_no . "_" . $i] ) ) {
				$noselect_text_array = explode(',', $_POST[$cols[27] . $manag_no . "_" . $i] );
				$noselect_text_array = array_filter( $noselect_text_array );
				$_POST[$cols[27] . $manag_no . "_" . $i] = implode( ',', $noselect_text_array );
			}
			...
			if ( isset( $_POST[$cols[36] . $manag_no . "_" . $i] ) !== false && '' !== ( $_POST[$cols[36] . $manag_no . "_" . $i] ) ) {
					...
					for ( $ii = 0; $cnt = count( $lines ), $ii < $cnt; $ii++ ) {
						...
						$lineContent = trim( $lines[$ii] );
						$lineContent = str_replace( '\\', '', $lineContent );
						$lineLen = strlen( $lineContent );
						$isQuoted = strstr( $lineContent, '"' );

						if ( false !== $isQuoted ) {

							$options[$ii]['value'] = str_replace( '"', '', $isQuoted );
							$quotedLen = strlen( $isQuoted );

							if ( $lineLen !== $quotedLen ) {
								$contents = explode( ":", $lineContent );
								$options[$ii]['text'] = str_replace( '--', '', $contents[0] );
							} else {
								$options[$ii]['text'] = $options[$ii]['value'];
							}
							...
							if ( 1 === count( $contents ) ) {
								$options[$ii]['value'] = str_replace( '--', '', $contents[0] );
							}
							// それ以外は「:」の後半を値として渡す
							else {
			...
			if ( isset( $_POST[$cols[39] . $manag_no . "_" . $i] ) !== false && '' !== ( $_POST[$cols[39] . $manag_no . "_" . $i] ) ) {
				$_POST[$cols[39] . $manag_no . "_" . $i] = str_replace(array(' ', '　'), '', $_POST[$cols[39] . $manag_no . "_" . $i] );
			}

			/**
			 *
			 * 各検索項目の保存前のデータ調整時に他のプログラムによって差し込まれる処理用のフック
			...
			$_POST = apply_filters( 'feas_admin_manag_adjust_param', $_POST, $args );
		}
...
*/
	...
	/**
	 *	検索結果の並び順 > 第一条件 > カスタムフィールドのキー
	 */
	if ( ! isset( $_POST[$feadvns_sort_target_cfkey . $manag_no] ) ) {

		$sort_target_cfkey = get_option( $feadvns_sort_target_cfkey . $manag_no );

		$_POST[$feadvns_sort_target_cfkey . $manag_no] = $sort_target_cfkey;
	}
	...
	if ( ! isset( $_POST[$feadvns_sort_target_cfkey_2nd . $manag_no] ) ) {
		...
		$sort_target_cfkey_2nd = get_option( $feadvns_sort_target_cfkey_2nd . $manag_no );

		$_POST[$feadvns_sort_target_cfkey_2nd . $manag_no] = $sort_target_cfkey_2nd;
	}
	...
	if ( ! isset( $_POST[$feadvns_reset_btn_sw . $manag_no] ) ) {
		...
		$_POST[$feadvns_reset_btn_sw . $manag_no] = $reset_btn_sw;
	}
	...
	if ( ! isset( $_POST[$feadvns_reset_btn_js .$manag_no] ) ) {
		...
		$_POST[$feadvns_reset_btn_js . $manag_no] = $reset_btn_js;
	}
	...
	if ( ! isset( $_POST[$feadvns_reset_btn_position . $manag_no] ) ) {
		...
		$_POST[$feadvns_reset_btn_position . $manag_no] = $reset_btn_position;
	}
	...
	if ( ! isset( $_POST[$feadvns_reset_btn_text . $manag_no] ) ) {
		...
		$_POST[$feadvns_reset_btn_text . $manag_no] = $reset_btn_text;
	}
	...
	if ( ! isset( $_POST[$pv_css . $manag_no] ) ) {
		...
		$pv_css_sw = get_option( $pv_css . $manag_no );

		$_POST[$pv_css . $manag_no] = $pv_css_sw;
	}
