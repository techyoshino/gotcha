...
function search_where_add( $where ) {
	...
	if ( isset( $_GET['csp']) && $_GET['csp'] == "search_add" && $wp_query->is_main_query() ) {
		...
		if ( $default_taxonomy ) {
			$cwhere = implode( ',', (array) $default_taxonomy );
		 ...
		 */
		if ( $exclude_post_ids ) {
			$exclude_post_ids = implode( ',', (array) $exclude_post_ids );
...
SQL;
		...
		if ( NULL !== $lang_id ) {
			$sql = " AND ( {$wpdb->term_relationships}.term_taxonomy_id IN (%d) )";
			$lang_sql = $wpdb->prepare( $sql, $lang_id );
		}
		...
		if ( $add_where != '' || $w_keyword != '' ) {
			...
			$ret_where = $lang_sql . $exid . $exclude_post_ids . $add_where . $w_keyword . $cwhere . $where;
			...
			if ( '1' === $ereq ) {
				$ret_where = $where . $exid . $exclude_post_ids . $cwhere;
			} else {
...
SQL;
				...
				if ( 'a' === $get_ele_value || 'd' === $get_ele_value || '1' == $get_ele_value || '4' == $get_ele_value ) {
					...
					if ( $range_value ) {
						...
						$meta_datas[$meta_cnt]['text']  = $range_value;
					}
					elseif ( $meta_value ) {
						if ( array_key_exists( $meta_value, $choices ) ) {
							$metaLabel = $choices[$meta_value];
						} else {
							$metaLabel = $meta_value;
						}
						$meta_datas[$meta_cnt]['metas'] = esc_sql( $meta_value );
						$meta_datas[$meta_cnt]['text']  = $metaLabel;
					}
				 ...
				 */
					...
					if ( $meta_value ) {
						$raw_text = $esq_text = $metaLabel = array();
						for ( $i = 0, $i_max = count( $meta_value ); $i < $i_max; $i++ ) {
							if ( array_key_exists( $meta_value[$i], $choices ) ) {
								$metaLabel[$i] = $choices[$meta_value[$i]];
							} else {
								$metaLabel[$i] = $meta_value[$i];
							}
							$raw_text[] = $meta_value[$i];
							$esq_text[] = $meta_value[$i];
						}
						$meta_datas[$meta_cnt]['metas']  = $esq_text; // meta_valueの配列
						$meta_datas[$meta_cnt]['text']   = $metaLabel; // meta_valueの配列
				...
				$meta_datas[$meta_cnt]['acf_plural'] = get_option( $cols[38] . $manag_no . '_' . $line_cnt );
				$meta_cnt++;
			}
		}
...
function create_where_single_cat( $data ) {
	...
	if ( array_key_exists( 'notin', $data ) && $data['notin'] != '' ) {
		...
		$sql .= " WHERE tt.term_id IN (" . esc_sql( $data['cat'] ) . ")";
	}
	$get_ids = $wpdb->get_results( $sql );
...
function create_where_keyword( $keywords, $kwds_target = '', $kwds_yuragi = 'yes', $manag_no, $number, $format = 'sql' ) {
	...
	if ( in_array( 'attachment', $target_pt_tmp ) ) {
		$target_status = "'publish', 'inherit'";
	} else {
		$target_status = "'publish'";
	}

	$post_sql .= " AND {$wpdb->posts}.post_status IN ( {$target_status} )";
		...
		for ( $i_tg = 0 , $i_cnt = 0; $i_tg < $tg_cnt; $i_tg++ ) {
			...
			if ( 'post_title' === $kt[$i_tg] ) {
				foreach ( $kwds[$i_key] as $word ) {
					...
					/**
					 * 「検索対象の投稿タイプ」に"メディア"が指定されている場合
					 * MacのSafari以外からアップロードされた、ファイル名に濁音/半濁音が含まれるファイルはそのままではヒットしないため、Normalizationにより正規化
					 */
					if ( in_array( 'attachment', $target_pt_tmp ) ) {
						$sql .= '( ';
						if ( function_exists( 'normalizer_normalize' ) ) {
							$sql .= "{$wpdb->posts}.post_title LIKE '%{$word}%' ";
							if ( ! normalizer_is_normalized( $word, Normalizer::FORM_C ) ) {
								$word = normalizer_normalize( $word, Normalizer::FORM_C );
								$sql .= "OR {$wpdb->posts}.post_title LIKE '%{$word}%' ";
							} elseif ( ! normalizer_is_normalized( $word, Normalizer::FORM_D ) ) {
								$word = normalizer_normalize( $word, Normalizer::FORM_D );
								$sql .= "OR {$wpdb->posts}.post_title LIKE '%{$word}%' ";
							}
						} else {
							$sql .= "{$wpdb->posts}.post_title LIKE '%{$word}%' ";
...
function create_where_single_meta( $datas ) {
	...
	// シリアライズされた値にてエスケープされているため
	$datas['metas'] = str_replace( '\\', '', $datas['metas'] );
	...
	if ( 'yes' === $datas['free_word'] ) {

		$formatted_value = $keywords . $datas['unit'] . $rangeKey;
		...
		if ( 'b' === $datas['orderby'] ) {
			...
			// 「関連するターム」の場合
				...
				if ( 'yes' == $datas['kugiri'] && $value_is_int ) {
					$formatted_value = number_format( $datas['text'], 0, '.', ',' );

				// 区切り and 数値ではない = そのまま表示
				} elseif ( 'yes' == $datas['kugiri'] && false == $value_is_int ) {
					$formatted_value = $datas['text'];

				// 区切らない = カンマ区切りしないもの
				} else {
					$formatted_value = str_replace( ',', '', $datas['text'] );
				}
	...
	// カンマ付きの数字はカンマ削除して比較（入力値、DBともに）
		...
		$acf_flag = $datas['acf_plural'];

		// Advanced Custom Fields のserializeされた値の場合
		if ( $acf_flag ) {

			$word = esc_sql( $datas['metas'] );
			$sql .= " AND ( meta_value LIKE '%\"{$word}\"%' )";
...
function create_where_plural_meta( $datas ) {
	...
	$raw_text  = $datas['text']; // エスケープ処理されていない語句
	...
	for ( $i_data = 0, $cnt_data = count( $get_metas ); $i_data < $cnt_data; $i_data++ ) {
		...
		// シリアライズされた値にてエスケープされているため
		// → LIKE検索するようにしたので完全一致させる必要がなくなったためコメントアウト
		//$get_metas[$i_data] = str_replace( "\\", '', $get_metas[$i_data] );
		//$raw_text[$i_data] = str_replace( "\\", '', $raw_text[$i_data] );
		...
		$get_metas[$i_data] = maybe_unserialize($get_metas[$i_data]);
		if ( is_array( $get_metas[$i_data] ) ) {
			$get_metas[$i_data] = $get_metas[$i_data][0];
		}
		...
		if ( 'b' === $datas['orderby'] ) {
			...
			// 「関連するターム」の場合
				...
				// 区切り and 数値 = カンマ区切りしたもの
				if ( 'yes' == $datas['kugiri'] && $value_is_int ) {
					$formatted_value = number_format( $raw_text[$i_data], 0, '.', ',' );

				// 区切り and 数値ではない = そのまま表示
				} elseif ( 'yes' == $datas['kugiri'] && false == $value_is_int ) {
					//$formatted_value = $get_metas[$i_data];
					$formatted_value = $raw_text[$i_data];

				// 区切らない = カンマ除去したもの
				} else {
					//$formatted_value = $unformatted_value;
					$formatted_value = str_replace( ',', '', $raw_text[$i_data] );
				}
