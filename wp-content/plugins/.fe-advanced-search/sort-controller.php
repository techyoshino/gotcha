...
function custom_sort( $order = null ) {
	global $wpdb, $wp_query, $cols, $cols_order, $manag_no, $feadvns_sort_target, $feadvns_sort_order, $meta_sort_key, $feadvns_sort_target_cfkey_as, $feadvns_sort_target_2nd, $feadvns_sort_order_2nd, $feadvns_sort_target_cfkey_2nd, $feadvns_sort_target_cfkey_as_2nd, $feadvns_sort_by_posttype;
	 ...
	 */
	$sbpt_sort = '';
	$sbpt = get_option( $feadvns_sort_by_posttype . $manag_no );
	if ( '1' === $sbpt ) {
		$sbpt_sort = "{$wpdb->posts}.post_type, ";
	}
	...
	if ( false !== $sTarget ) {
		...
		switch( $sTarget ) {
			...
			case 'post_date': // 投稿日
				$sort_order  = "{$sbpt_sort}{$wpdb->posts}.post_date {$get_sort}";
				break;

			case 'post_title': // 投稿タイトル
				$sort_order  = "{$sbpt_sort}{$wpdb->posts}.post_title {$get_sort}";
				break;

			case 'post_name': // 投稿スラッグ
				$sort_order  = "{$sbpt_sort}{$wpdb->posts}.post_name {$get_sort}";
				break;

			case 'post_meta': // カスタムフィールド
				$sort_order  = "{$sbpt_sort}cf IS NULL ASC, ";
				$sort_order .= "cf ASC, ";
				if ( 'int' == $get_cfkey_as )
			...
			case 'menu_order': // 外部プラグイン（Intuitive Custom Post Orderなど）
				$sort_order  = "{$sbpt_sort}{$wpdb->posts}.menu_order {$get_sort}";
				break;

			case 'rand': // ランダム
				$sort_order = "{$sbpt_sort} RAND()";
/*
				$nowH = date_i18n( 'm' );
				if ( $nowH > 16 ) {
...
*/
			...
			default:
				$sort_order = "{$sbpt_sort}{$wpdb->posts}.post_date DESC";
				break;
		}
	...
	if ( false !== $sTarget_2nd ) {
		...
		switch( $sTarget_2nd ) {
			...
			case 'menu_order': // 外部プラグイン（Intuitive Custom Post Orderなど）
				$sort_order  = ", {$wpdb->posts}.menu_order {$get_sort}";
				break;
...
function sort_add_field( $get_sql_data = null ) {
	...
	if ( 'post_meta' === $sortTarget1st || 'post_meta' === $sortTarget2nd ) {
		...
		// WordPress 6.0 以降、SQLに改行とタグが含まれるため除去…
		$get_sql_data = str_replace( array( "\r\n", "\r", "\n" ), ' ', $get_sql_data );
		$get_sql_data = str_replace( array( "\t" ), '', $get_sql_data );
